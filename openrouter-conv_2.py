...
def sanitize_user_prompt(prompt: str) -> str:
    """
    Удаляет символы, которые могут вызвать JSONDecodeError при сериализации user_prompt.
    """
    # Удаляем управляющие символы за исключением \n, \r, \t
    cleaned = ''.join(
        ch for ch in prompt
        if ch in ('\n', '\r', '\t') or ord(ch) >= 0x20
    )
    ...
    try:
        ...
        # Удаляем невалидные символы Unicode (редко)
        cleaned = cleaned.encode('utf-8', 'ignore').decode('utf-8')
        return cleaned
    except Exception as e:
        # На случай других экзотических проблем
...
def call_openrouter(model, system_prompt: str, user_prompt: str, comment="") -> str:
    ...
    if not user_prompt:
        print("❗ Пустой user_prompt, пропускаем вызов модели.")
        return ""   
    while True:
        try:
            if ECHO_OUTPUT:
                ...
                # Обычный (нестриминговый) ответ — здесь доступны заголовки
                ...
                if hasattr(response, 'response') and hasattr(response.response, 'headers'):
                    ...
                    if limit and remaining and reset:
                        ...
                        print(f"🔄 Осталось запросов: {remaining} из {limit}. Сброс лимита в {reset_time}.")
            ...
            if reset_timestamp_ms:
                ...
                print(f"\n💡 Лимит исчерпан. Ожидаем до {reset_time_str} ({int(wait_time)} секунд)...")
                time.sleep(wait_time + 1)
            else:
                print("⛔ RateLimitError без X-RateLimit-Reset. Повтор через 60 секунд...")
                time.sleep(60)
...
def annotate_batch(index : int, batch: List[Dict[str, str]], headers: List[str]) -> List[Dict]:
    ...
    user_prompt = f"Разметь следующие строки, вот входной файл в формате csv:\n\n{csv_block}"
...
def main():
    ...
    for i, batch in enumerate(tqdm(batches, desc="Обработка пачек", unit="пачка")):
        ...
        if (i < start_batch):
            continue
        try :
            print(f"Обработка пачки {i+1} из {len(batches)}...")
            # Аннотация
            attempts = 0
            annotated = None
            while attempts < 5:
                ...
                if annotated and len(annotated) > 4000:
                    ...
                    """ try:
                        ...
                        print("Ошибка парсинга JSON при аннотации. Попытка "+ str(attempts + 1)) """
                attempts += 1
                current_time = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                print(f"[{current_time}] Ошибка аннотации пачки {i} (длина аннотации {len(annotated)}). Попытка {attempts} из 5. Повтор через {100 * attempts * attempts} секунд...")
                time.sleep(100 * attempts * attempts)  # Увеличиваем время ожидания при ошибке
            if not annotated:
                continue
        ...
        # errors = validate_annotations(i, annotated)
            ...
            print(f"Ошибка при обработке пачки {i}: {e}")
