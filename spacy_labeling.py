...
def build_spacy_ruler(nlp):
    ...
    patterns = [
        # Упаковка (х30, x10, №100)
        {"label": "ПотребительскаяУпаковкаКолво", "pattern": [
            {"TEXT": {"REGEX": r"^(x|х|№|№\s?|×|\*)"}}, {"TEXT": {"REGEX": r"\d+"}}
        ]},

        # Дозировка (400 мкг, 0.5 мг и т.п.)
        {"label": "Дозировка", "pattern": [
            {"TEXT": {"REGEX": r"^\d+([.,]\d+)?$"}}, {"TEXT": {"REGEX": r"^(мг|мкг|г|мл|ед|МЕ|%)$"}}
        ]},

        # ЛекФорма (табл п/о плен и вариации)
        {"label": "ЛекФорма", "pattern": [
            {"TEXT": {"REGEX": r"^(табл|капс|р-р|сусп|мазь|крем|гель|амп|саше|шпр|спрей)$"}},
            {"TEXT": {"REGEX": r"^(п/о|в/в|плен|д/инъекций|д/приема|жев|длительного|высвобожд)$"}, "OP": "*"}
        ]},

        # ТорговоеНаименование: любые 2-5 слов подряд, допускается цифра и строчные
        {"label": "ТорговоеНаименование", "pattern": [
            {"IS_TITLE": True, "OP": "?"},
            {"IS_ALPHA": True, "OP": "+"},
            {"IS_ALPHA": True, "OP": "*"}
        ]}
