...
def build_spacy_ruler(nlp):
    ...
    patterns = [
        {"label": "ТорговоеНаименование", "pattern": [{"TEXT": {"REGEX": r"^[А-ЯA-Z][а-яa-z\-]+"}}]},
        {"label": "Дозировка", "pattern": [{"TEXT": {"REGEX": r"\d+([.,]\d+)?"}}, {"TEXT": {"REGEX": r"мг|мкг|мл|%"}}]},
        {"label": "ЛекФорма", "pattern": [{"TEXT": {"REGEX": r"табл|капс|р-р|сусп|мазь|крем|гель|амп|саше|шпр|спрей"}}]},
        {"label": "ПотребительскаяУпаковкаКолво", "pattern": [{"TEXT": {"REGEX": r"№"}}, {"TEXT": {"REGEX": r"\d+"}}]},
    ]
...
if __name__ == "__main__":
    ...
    if args.mode == "annotate":
        ...
        print("📥 Чтение Excel...")
        texts = load_texts_from_excel(args.input_file, limit=100)
